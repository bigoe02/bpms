
 
 <div class=" sidebar" role="navigation">
            <div class="navbar-collapse">
        <nav class="cbp-spmenu cbp-spmenu-vertical cbp-spmenu-left" id="cbp-spmenu-s1">
          <ul class="nav" id="side-menu">
            <li>
              <a href="dashboard.php"><i class="fa fa-home nav_icon"></i>Dashboard</a>
            </li>
            <?php if (strlen($_SESSION['bpmsaid']==1)) {?>
            <li>
              <a href="add-services.php"><i class="fa fa-cogs nav_icon"></i>Services<span class="fa arrow"></span> </a>
              <ul class="nav nav-second-level collapse">
                <li>
                  <a href="add-services.php">Add Services</a>
                </li>
                <li>
                  <a href="manage-services.php">Manage Services</a>
                </li>
              </ul>
              <!-- /nav-second-level -->
            </li>
           
            <li class="">
              <a href="about-us.php"><i class="fa fa-book nav_icon"></i>Pages <span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
                <li>
                  <a href="about-us.php">About Us</a>
                </li>
                <li>
                  <a href="contact-us.php">Contact Us</a>
                </li>
              </ul>
              <!-- /nav-second-level -->
            </li>
            <?php }?>
            <li>
              <a href="all-appointment.php"><i class="fa fa-check-square-o nav_icon"></i>Appointment<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
                <li>
                  <a href="all-appointment.php">All Appointment</a>
                </li>
                <li>
                  <a href="new-appointment.php">New Appointment</a>
                </li>
                <li>
                  <a href="accepted-appointment.php">Accepted Appointment</a>
                </li>
                <li>
                  <a href="rejected-appointment.php">Rejected Appointment</a>
                </li>
                <li>
                  <a href="completed-appointment.php">Completed Appointment</a>
                </li>
              </ul>
              <!-- //nav-second-level -->
            </li>
           
        
           <li>
              <a href="readenq.php"><i class="fa fa-check-square-o nav_icon"></i>Enquiry<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
                <li><a href="readenq.php">Read Enquiry</a></li>
                <li><a href="unreadenq.php">Unread Enquiry</a></li>
               
              </ul>
              <!-- //nav-second-level -->
            </li>
             <li>
              <a href="customer-list.php" class="chart-nav"><i class="fa fa-users nav_icon"></i>Customer<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
              <li><a href="customer-list.php" >Customer List</a></a></li>  
              <li><a href="feedbackpage.php">Customer Feedback</a></a></li>
               
              </ul>
              <!-- //nav-second-level -->
            </li>
            <li>
              <a href="employee-leaverequest.php" class="chart-nav"><i class="fa fa-group nav_icon"></i>Employees<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
              <?php if (strlen($_SESSION['bpmsaid']==1)) {?><li><a href="employee-list.php" >Employee List</a></li> 
              <li><a href="employee-payroll.php">Payroll</a></li>   <?php }?>
              <li><a href="employee-performance.php?viewid=<?php echo  $_SESSION['bpmsid']; ?>" >Your Performance</a></li> 
              <li><a href="employee-leaverequest.php">Leave Request</a></li> 
             
               
              </ul>
              <!-- //nav-second-level -->
            </li>
            <li>
              <a href="products.php"><i class="fa fa-check-square-o nav_icon"></i>Inventory<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse" >
              <li><a href="products.php"> Add Products / Stocks</a></li>
              <li><a href="products-categories.php"> Product Categories </a></li>
                 <li><a href="products-suppliers.php"> Supplier</a></li>
      
              </ul>
              <!-- //nav-second-level -->
            </li>
            <?php if (strlen($_SESSION['bpmsaid']==1)) {?>
              <li>
              <a href="bwdates-reports-ds.php"><i class="fa fa-check-square-o nav_icon"></i>Reports<span class="fa arrow"></span></a>
              <ul class="nav nav-second-level collapse">
                 <li><a href="bwdates-reports-ds.php"> B/w dates</a></li>
                   
                    <li><a href="sales-reports.php">Sales Reports</a></li>
                    <li><a href="invoices.php">Invoices</a></li>
              </ul>
              <!-- //nav-second-level -->
            </li>
            <?php }?>

 
            <!-- <li>
              <a href="search-appointment.php" class="chart-nav"><i class="fa fa-search nav_icon"></i>Search Appointment</a>
            </li>
            <li>
              <a href="search-invoices.php" class="chart-nav"><i class="fa fa-search nav_icon"></i>Search Invoice</a>
            </li> -->
          

          </ul>
          <div class="clearfix"> </div>
          <!-- //sidebar-collapse -->
        </nav>
      </div>
    </div>

