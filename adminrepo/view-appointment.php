...
if(isset($_POST['submit']))
  {
   ...
   $query=mysqli_query($con, "update  tblbook set Remark='$remark',Status='$status' where ID='$cid'");
    if ($query) {
      ...
      $alertSuccessMessage = "<script>showAlert('Success', 'ALL MARK HAS BEEN UPDATED', 'success');</script>";
      echo "<script type='text/javascript'> setTimeout(function(){ document.location ='all-appointment.php'; }, 8000); </script>";
  }
  else
    {
      $alertSuccessMessage = "<script>showAlert('Error', 'Something went wrong! Please Try Again.', 'error');</script>";
    }
}
  ?>
<!DOCTYPE HTML>
...
tblbook.Remark,
tblbook.Status,
tblbook.RemarkDate from tblbook 
join tbluser on 
tbluser.ID=tblbook.UserID where tblbook.ID='$cid'");
$cnt=1;
